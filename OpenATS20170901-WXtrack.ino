...
//更精准的浮点数，而网上有WXtrack 3.8.28版本的破解注册机，需要破解后也可以完成，但破解对于原作者的贡献实在是愧疚
...
//软件注册后的串口数据格式为：AZ84.46 EL19.41\#13
 ...
 if (usbdata.length() > 10)
   {

      const char *usb=usbdata.c_str();                            //将string数据转成字符串数据
      //Serial.println(usb);
      sscanf(usb, "AZ%s EL%s\#13", &a, &b);                       //WXtrack 控制数据提取
      //sscanf(usb, "AZ:%s,EL:%s", &a, &b);                       //not split out AZ and EL ---DDE client 
      //sscanf(usb, "UP0 DN%s UMFM-N DMFM-N AZ%s EL%s SNNOAA-%s\#0\#13\#10", &g,&a,&b,&h);    //no anythins  ---DDE client
      //sscanf(usb, "AZ:%s,EL:%s\#13\#10", &a, &b);                //send newline and split out AZ and EL---DDE client
      tmp_a = a;                                                
      tmp_b = b;                                                 
 ...
 //程序自动追踪代码结束
 ///////////////////////////////////////////////////////////////////////////////////////////////////////////////
  else if(usbdata.startsWith("W") && power != 1)                                 //检测是否为天线唤醒命令“W”
  {     
...
//否则进行手动控制
 ...
 else if ( usbdata.length() > 0 && power == 0 )
      {
         Serial.println("#############################################################"); 
     ...
     {
         Serial.println("######################################################"); 
         Serial.println("The antenna is LOCKED , Please send 'W' to UNLOCK him !");
         Serial.println("######################################################"); 
     }  
//方位角旋转角度转换
...
if ( power_tmp == 1 && power == 1)                                //执行天线断电程序
  {
